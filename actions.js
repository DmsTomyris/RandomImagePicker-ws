...
const ROUNDS = 1; // Amount of rounds the carousel will shift through
const CAROUSEL_TIME = 2; // Total time in seconds carousel will spin

// Audio-Elemente für die Sounds
const tickSound = new Audio("winner.mp3");
tickSound.loop = true; // Tick-Sound in einer Schleife abspielen
const taDaSound = new Audio("ta-da.mp3"); // Ta-Da-Sound für das finale Bild
...
function doCarouselRec(index, durations, deleteImage) {
    ...
    if (durations.length > 0) {
        ...
        // Finales Bild anzeigen und Tick-Sound stoppen
        setFinalImage(index, deleteImage);
        tickSound.pause();
        tickSound.currentTime = 0; // Sound zurücksetzen

        // Ta-Da-Sound abspielen
        taDaSound.play();
    }
}
...
function f(x, steps) {
    ...
    return -a * Math.log(x) + c * 0.01; // Verringert die Anzeigedauer um die Hälfte
}
